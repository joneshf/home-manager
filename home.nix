...

{
  ...
  home = {
    ...
    sessionVariables = {
      ...
      # Version 0.29.0 of `jj` reports a warning for every command if the configuration file is in the pre-0.29.0 location.
      # The configuration file is generated by `programs.jujutsu.settings`.
      # This fixed on the `master` branch of `home-manager`,
      # but hasn't been backported to the `24.11` branch.
      #
      # We can silence the warning for now by explicitly setting the `JJ_CONFIG` environment variable to point to the pre-0.29.0 location.
      # We should be able to remove this once we update `home-manager` to `25.05` or newer.
      JJ_CONFIG = "${home-directory}/Library/Application Support/jj/config.toml";
    };
